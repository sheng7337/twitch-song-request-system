  ...
  en: {
    ...
    twitchApp: {
      ...
      steps: [
        ...
        'Client Type: <strong>Public</strong> (not Confidential — no Client Secret needed) → click Create',
      ...
      hint: 'This is a public identifier — safe to share. No Client Secret needed.',
    ...
    rewards: {
      ...
      songHint: 'Viewers type a song title when they redeem this — only rewards with text input are shown here.',
      randomLabel: '🎲 Random Song',
      randomHint: 'Picks a random song from your list automatically — only rewards without text input are shown here.',
      noRewardsTitle: 'No rewards found',
      noRewardsBody: 'Channel may not be Affiliate yet, or try refreshing.',
      noMatchingTitle: 'No matching rewards',
      noMatchingBody: (requiresText) => requiresText
        ? 'None of your existing rewards ask viewers to type something, so none can be used for Song Request (it needs a text box). Create a new one below.'
        : 'All of your existing rewards ask viewers to type something, so none can be used for Random Song (it must NOT have a text box). Create a new one below.',
  ...
  'zh-TW': {
    ...
    twitchApp: {
      ...
      steps: [
        ...
        '用戶端類型（Client Type）：選 <strong>Public</strong>（不要選 Confidential —— 不需要 Client Secret）→ 點擊 Create',
        '點擊你新建立的應用程式上的 <strong>[Manage]（管理）</strong>',
        '複製最上方顯示的 <strong>Client ID</strong>',
      ],
      fieldLabel: 'Client ID',
      placeholder: '例如 abc123def456ghi789',
      hint: '這是公開的識別碼，可以安全分享，不需要 Client Secret。',
      verifyBtn: '驗證並繼續 →',
    ...
    rewards: {
      ...
      songLabel: '🎵 點歌券',
      songHint: '觀眾兌換時需要輸入歌曲名稱 —— 這裡只會顯示「需要輸入文字」的兌換項目。',
      randomLabel: '🎲 隨機點歌',
      randomHint: '會自動從你的歌單中隨機挑選一首歌曲 —— 這裡只會顯示「不需要輸入文字」的兌換項目。',
      noRewardsTitle: '找不到任何兌換項目',
      noRewardsBody: '你的頻道可能尚未開通附屬主播（Affiliate）資格，或請嘗試重新整理。',
      noMatchingTitle: '沒有符合的兌換項目',
      noMatchingBody: (requiresText) => requiresText
        ? '你目前的兌換項目都不需要輸入文字，所以都無法用於「點歌券」（它需要文字輸入框）。請在下方建立新的兌換項目。'
        : '你目前的兌換項目都需要輸入文字，所以都無法用於「隨機點歌」（它不能有文字輸入框）。請在下方建立新的兌換項目。',
      textInputTag: '[需輸入文字]',
      noTextTag: '[免輸入]',
      newTag: '[新建]',
