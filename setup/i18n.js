  ...
  en: {
    ...
    twitchApp: {
      ...
      steps: [
        ...
        'Category: <strong>Other</strong> → click Create',
    ...
    songSheet: {
      ...
      pickColumnsHint: 'Click the column headers to identify each one:',
      ...
      selectKeyHint: 'Click the key/transpose column (optional) or click Confirm Columns to skip',
      selectedSummary: (song, artist, key) => `✓ Song: "${song}", Artist: "${artist}"${key ? `, Key: "${key}"` : ''}`,
  ...
  'zh-TW': {
    ...
    twitchApp: {
      ...
      steps: [
        ...
        '分類（Category）：選 <strong>Other</strong> → 點擊 Create',
    ...
    songSheet: {
      ...
      labelArtist: '歌手名稱',
      labelKey: 'Key（選填）',
      selectKeyHint: '請點擊 Key / 升降 Key 欄位（選填），或直接點擊「確認欄位」跳過',
      selectedSummary: (song, artist, key) => `✓ 歌曲：「${song}」，歌手：「${artist}」${key ? `，Key：「${key}」` : ''}`,
